# (c) Giles Cooper 2024
"""
1.)  Initial comment w/ data set name, and what you are doing
2.)  Open the file, input and store the data in a Panda dataset.
3.)  Remove any columns that are irrelevant to your study
4.)  Remove any rows with bad data that may affect your results
5.)  Perform the operations necessary for the analyses described in Project 6.
       Be sure the your code is well commented.
6.) Be sure that your work includes at least 6 views of the data and 2 plots. 
"""
##############################################################################
"""
I will be working with the dataset of Dungeons & Dragons 5th edition monsters.

I had some ideas of what views I might display and what plots I might do,
    but I thought it might just be more interesting to follow where the data 
    goes; that is to say, begin by parsing the text fields into standardized, 
    granular forms, and then continue investigating views of anomalies that 
    come up. Ideally, there is no "bad data" if it is properly parsed and 
    normalized, and in this case parsing ended up being a significant portion 
    of the program.
Ultimately the views and plots are about the race/type of creature and their
    moral alignment, and the creature's HP and challenge rating and how they 
...
def main(df):
#         drop unused columns in order to satisfy assignment requirement 3.):
    df.drop(['Armor', 'Speed'], axis='columns')
#   ^ ^ ^ REMEMBER TO REMOVE THIS LINE WHEN USING THIS PROGRAM IN FUTURE ^ ^ ^

    print(f"\nTotal records in D&D 5e Monster Manual Dataset: {len(df)}\n")
    ...
    print("List of Troublesome Data Monsters:")
...
if __name__=="__main__":
    try:
        with open(FILE, 'r') as f: df = pd.read_csv(f)
    except Exception as e:
        print(f"FILE NOT FOUND: {FILE}\n{e}")
    else:
        main(df)
# I think quit() and/or exit() have different effects on different systems;
# I use linux. This is outside the with-open block so the file 'should' be
# closed, but, for the sake of good coding habit I'll abstain from dumping
# the user uncerimoniously to terminal with quit() when the file isn't found
# 'BuT iT wOrKeD oN My CoMPuTeR!!'


"""There's definitely a lot more to work with on this data set. We might split
    the Speed column into the various forms of movement. A comparison of size
...
I spent much more time parsing data than I did actually plotting or displaying 
    ...
    many datasets, especially big CSVs. Querying from relational databases might
    not have that same issue because the data would be normalized or whatever.
